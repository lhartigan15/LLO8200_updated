...
```{r}
#my dataset didn't have the got_pizza_f variable; so I added it
za$got_pizza_f<-as.factor(za$got_pizza)
```
...
logit_results%>%
  predict(za_test,type="prob")%>%
  bind_cols(za_test)%>%
  roc_auc(truth=got_pizza_f,.estimate=.pred_1, event_level="second")
```
